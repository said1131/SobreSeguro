...
const isResidual = (nombre: string): boolean => nombre.toLowerCase() === "residuo";
...
export const obtenerIngreso = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const ingresos = await obtenerMovimientosUsuario(usuario.id, "Ingreso");
...
export const actualizarIngreso = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const sobresUsuario = await obtenerSobresUsuario(usuario.id, usuario.email);
        ...
        await registrarMovimiento(usuario.id, "Ingreso", monto, "Ingreso general", null, null);
        ...
        await actualizarSaldoAhorro(ahorro.id, nuevoSaldoAhorro);
        ...
        if (sobreResidual) {
            ...
            await actualizarSaldoSobre(sobreResidual.id, saldoResidual);
        } else if (montoResidual > 0) {
            await crearSobreUsuario(usuario.id, "Residuo", porcentajeRestante, montoResidual);
        }

        const sobresActualizados = await obtenerSobresUsuario(usuario.id, usuario.email);
...
export const obtenerHistorialCompleto = async (req: Request, res: Response): Promise<void> => {
    const usuarioEmail = req.headers["x-usuario-email"] as string;
    ...
    try {
        ...
        const movimientos = await obtenerMovimientosUsuario(usuario.id);
        const sobres = await obtenerSobresUsuario(usuario.id, usuario.email, true);

        const nombreSobrePorId = new Map<number, string>();
        const nombreAhorroPorId = new Map<number, string>();
